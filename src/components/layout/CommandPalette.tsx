...
import { tools, searchTools, getToolById } from '@/tools/registry';
...
export function CommandPalette() {
  ...
  const filteredTools = React.useMemo(() => {
    if (!query) {
      const favTools = favorites.map(id => getToolById(id)).filter(Boolean) as typeof tools;
      const recTools = recentTools.filter(id => !favorites.includes(id)).map(id => getToolById(id)).filter(Boolean) as typeof tools;
